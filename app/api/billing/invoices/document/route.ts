...
export async function GET(request: Request) {
  ...
  const { data: invoice, error } = await supabaseAdmin
    ...
    .select(
      "invoice_number, description, plan_name, charge_type, issue_date, due_date, period_start, period_end, subtotal, vat_amount, total_amount, amount_paid, balance_due, status, schools(school_name)"
    )
  ...
  <html lang="en">
    <head>
      ...
      <style>
        *{box-sizing:border-box} body{margin:0;background:#fff8f2;color:#2d2a3e;font-family:Arial,sans-serif}
        ...
        h1{margin:0;font-size:30px}.bloom{color:#ff5ea8}.doc{text-align:right}.doc h2{margin:0 0 8px}
    ...
    <body>
      ...
      <main class="page">
        <div class="head">
          <div><h1>Daily<span class="bloom">Bloom</span></h1><p class="muted">Preschool Management Platform</p></div>
