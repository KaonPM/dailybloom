...
export async function GET(request: Request) {
  ...
  const [{ data: enquiry }, { data: school }, { data: settings }, { data: configuration }, { data: documents }, { data: requirements }, { data: consents }, { data: terms }, { data: fees }] = await Promise.all([
  ...
  return NextResponse.json({ enquiry, school, settings, configuration, fees: fees || [], documents: documents || [], requirements: requirements || [], consents: consents || [], terms: termsWithFees(terms, fees) });
}
