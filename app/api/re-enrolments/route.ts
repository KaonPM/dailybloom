...
type ReenrolmentAction = "create_campaign" | "send_notifications" | "approve_reenrolment" | "decline_reenrolment";
...
export async function GET(request: Request) {
  ...
  const [schoolResult, feeResult, campaignsResult] = await Promise.all([
    ...
    supabaseAdmin.from("school_reenrolment_campaigns").select("id, school_year, source_form_id, registration_fee_type_id, registration_fee_amount, response_deadline, status, created_at").eq("school_id", schoolId).order("school_year", { ascending: false }),
  ]);
  const failure = schoolResult.error || feeResult.error || campaignsResult.error;
  ...
  if (campaign?.id) {
    const recordsResult = await supabaseAdmin
      ...
      .select("id, learner_id, reenrolment_reference, parent_portal_phone, registration_fee_amount, registration_payment_status, status, notification_sent_at, notification_error, submitted_data, decline_reason, created_at")
    ...
    reenrolments = records.map((record) => {
      ...
      const submittedData = record.submitted_data && typeof record.submitted_data === "object"
        ? record.submitted_data as { parent_notes?: unknown }
        : {};
...
export async function POST(request: Request) {
  ...
  if (action === "create_campaign") {
    ...
    const [openCampaignResult, feeResult] = await Promise.all([
    ...
    if (openCampaignResult.error || feeResult.error) return NextResponse.json({ error: openCampaignResult.error?.message || feeResult.error?.message }, { status: 500 });
  ...
  if (action === "approve_reenrolment" || action === "decline_reenrolment") {
    ...
    const recordResult = await supabaseAdmin
      .from("learner_reenrolments")
      .select("id, learner_id, status, parent_portal_phone, registration_fee_amount")
      .eq("id", reenrolmentId)
      .eq("school_id", schoolId)
      .maybeSingle();
    ...
    if (action === "approve_reenrolment") {
      ...
      const updateResult = await supabaseAdmin
        .from("learner_reenrolments")
        .update({ status: "approved", reviewed_by: access.staff.userId, reviewed_at: reviewedAt, decline_reason: null, updated_at: reviewedAt })
        .eq("id", reenrolmentId)
        .eq("status", "submitted");
      if (updateResult.error) return NextResponse.json({ error: updateResult.error.message }, { status: 500 });
      ...
      return NextResponse.json({ success: true, message: "Re-enrolment approved. The confirmed Parent Portal number has been applied when it changed." });
    }
