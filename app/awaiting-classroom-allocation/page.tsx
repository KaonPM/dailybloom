...
type Classroom = { id: number; classroom_name: string; age_groups?: string[] | null };
...
type ApprovedReenrolment = { id: string; learner_id: string; learner_name: string; classroom_name: string; reenrolment_reference: string; school_year: number; status: string; next_classroom_id?: number | null };
type AllocationData = { campaign: { id: string; school_year: number } | null; classrooms: Classroom[]; approved_enrolments: ApprovedEnrolment[]; reenrolments: ApprovedReenrolment[] };
...
export default function AwaitingClassroomAllocationPage() {
  ...
  async function autoAllocateReenrolments() {
    ...
    try {
      const response = await authenticatedFetch("/api/re-enrolments", {
        ...
        body: JSON.stringify({ action: "auto_allocate_reenrolments", school_id: schoolId, campaign_id: data.campaign.id }),
      });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error || "Automatic classroom allocation failed.");
      setMessage(`${body.message || "Automatic classroom allocation completed."}${body.awaiting_manual ? ` ${body.awaiting_manual} learner${body.awaiting_manual === 1 ? " needs" : "s need"} manual allocation because no matching classroom age group was found.` : ""}`);
      await load(schoolId);
    } catch (allocationError) { setError(allocationError instanceof Error ? allocationError.message : "Automatic classroom allocation failed."); }
    finally { setSaving(false); }
  }

  const enrolments = data?.approved_enrolments.filter((item) => !item.placement?.classroom_id) || [];
  const reenrolments = data?.reenrolments.filter((item) => item.status === "approved" && !item.next_classroom_id && Date.now() >= new Date(`${item.school_year}-01-01T00:00:00+02:00`).getTime()) || [];
  ...
  return <main style={{ maxWidth: 1280, margin: "0 auto", padding: "24px 18px 44px", display: "grid", gap: 18 }}>
    <section className="db-card db-card-green" style={{ padding: 18 }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, flexWrap: "wrap" }}><div><p className="db-eyebrow">School Management</p><h1 className="db-page-title" style={{ marginBottom: 6 }}>Awaiting Classroom Allocation</h1><p className="db-page-subtitle">Allocate approved learners to classrooms.</p></div><div className="db-page-actions">{reenrolments.length && data?.campaign ? <button className="db-button-primary" type="button" disabled={saving} onClick={() => void autoAllocateReenrolments()}>{saving ? "Allocating…" : "Auto-allocate re-enrolments"}</button> : null}<Link className="db-main-pill db-main-pill-yellow" href={`/dashboard${schoolQuery}`}>Dashboard</Link><Link className="db-button-secondary" href={`/re-enrolments${schoolQuery}`}>Re-enrolments</Link></div></div></section>
