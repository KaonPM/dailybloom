...
const sources: Array<["All" | RenewalSource, string]> = [["All", "All sources"], ["registration", "Registration"], ["document", "Documents & Evidence"], ["staff", "Staff Compliance"], ["manual", "Recorded Certificate"]];
...
export default function CertificatesPage() {
  ...
  const [schoolId, setSchoolId] = useState<number>(); const [data, setData] = useState<Response>(); const [documentOptions, setDocumentOptions] = useState<DocumentOption[]>([]); const [loading, setLoading] = useState(true);
  ...
  const [showNoExpiry, setShowNoExpiry] = useState(true); const [showManual, setShowManual] = useState(false); const [manualToEdit, setManualToEdit] = useState<RenewalItem | null>(null); const [saving, setSaving] = useState(false);

  const load = async (id: number) => {
    const [response, documentsResponse] = await Promise.all([authenticatedFetch(`/api/compliance/renewals?school_id=${id}`), authenticatedFetch(`/api/dbe-compliance-documents?school_id=${id}`)]); const result = await response.json(); const documentsResult = await documentsResponse.json();
    if (!response.ok || !documentsResponse.ok) throw new Error(result.error || documentsResult.error || "Certificates and renewals could not be loaded.");
    setData(result); setDocumentOptions(documentsResult.documents || []);
  };
  ...
  return <div><ComplianceHeader title="Certificates & Renewals" description="Track renewal dates from registration, uploaded documents, staff records and recorded certificate details." />
    ...
    <section className="db-card db-card-blue" style={{ padding: 16, marginTop: 14 }}><div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap", alignItems: "center" }}><div><h3 style={{ margin: 0 }}>What expires next?</h3><p className="db-helper" style={{ marginBottom: 0 }}>Upload files in Documents &amp; Evidence. Use this form only to record certificate details and renewal dates.</p></div><button className="db-button-secondary" onClick={() => { setManualToEdit(null); setShowManual((value) => !value); }}>{showManual ? "Close certificate details" : "Record certificate details"}</button></div>
      {showManual ? <form key={manualToEdit?.id || "new"} onSubmit={addManual} style={{ display: "grid", gap: 10, marginTop: 14 }}><strong>{manualToEdit ? "Manage certificate details" : "Record certificate details"}</strong><p className="db-helper" style={{ margin: 0 }}>Certificate details are entered manually. DailyBloom does not automatically read the uploaded document. Confirm every value against the original certificate before saving.</p><div style={formGrid}><label>Certificate name<input name="certificate_type" className="db-input" required maxLength={160} defaultValue={manualToEdit?.title || ""} /></label><label>Belongs to / holder<input name="holder_name" className="db-input" maxLength={160} placeholder="School or staff member" defaultValue={manualToEdit?.holder === "School" ? "" : manualToEdit?.holder || ""} /></label><label>Issue date<input name="issue_date" className="db-input" type="date" defaultValue={manualToEdit?.issue_date || ""} /></label><label>Expiry date<input name="expiry_date" className="db-input" type="date" defaultValue={manualToEdit?.expiry_date || ""} /></label>{!manualToEdit ? <><label>Issuing authority<input name="issuing_authority" className="db-input" maxLength={160} /></label><label>Reference<input name="certificate_reference" className="db-input" maxLength={160} /></label></> : null}<label>Linked document<select name="document_id" className="db-input" defaultValue={manualToEdit?.linked_document_id || ""}><option value="">No document linked</option>{documentOptions.map((document) => <option key={document.id} value={document.id}>{document.document_name}</option>)}</select></label><label>Renewal status<select name="renewal_status" className="db-input" defaultValue={manualToEdit?.renewal_status || "Current"}><option>Current</option><option>Not Started</option><option>In Progress</option><option>Renewed</option></select></label></div><Link className="db-button-secondary" href={route("/dbe-registration/documents")}>Upload in Documents &amp; Evidence</Link>{!manualToEdit ? <label>Notes<textarea name="notes" className="db-input" maxLength={1000} /></label> : null}<div><button className="db-button-primary" disabled={saving}>{saving ? "Saving..." : manualToEdit ? "Save renewal" : "Save certificate details"}</button></div></form> : null}</section>
