...
type PrintableEnrolment = {
  ...
  school?: { school_name?: string | null; logo_url?: string | null; registration_number?: string | null; contact_number?: string | null; email_address?: string | null; physical_address?: string | null } | null;
...
export default function PrintBlankEnrolmentPage() {
  ...
  return <main className="db-public-page blank-enrolment-print"><section className="db-card blank-form-sheet">
    <header className="blank-form-header"><div className="blank-form-school-details"><p><strong>Registration / EMIS / NPO number:</strong> {data.school?.registration_number || "Not provided by the school"}</p><p><strong>Registered address:</strong> {data.school?.physical_address || "Not provided by the school"}</p></div><div className="blank-form-brand">{data.school?.logo_url ? <img src={data.school.logo_url} alt={`${data.school.school_name || "School"} logo`} /> : <div className="blank-logo-placeholder">School logo</div>}<h1>{data.school?.school_name || "School"}</h1><p>{configuration.form_title || "Enrolment Form"}</p></div><div className="blank-form-contacts"><p><strong>Contact number:</strong> {data.school?.contact_number || "Not provided by the school"}</p><p><strong>Email:</strong> {data.school?.email_address || "Not provided by the school"}</p></div></header>
    <div className="blank-form-actions no-print"><Link className="db-button-secondary" href={enrolmentsHref}>Back to Enrolments</Link><button className="db-button-primary" type="button" onClick={() => window.print()}>Print form</button></div>
    ...
    {data.terms.length ? <section><h2>Terms & conditions</h2>{data.terms.map((item) => <div className="blank-term" key={item.title}><strong>{item.title}</strong><p>{item.content}</p></div>)}<p>☐ I accept the terms and conditions.</p></section> : null}
