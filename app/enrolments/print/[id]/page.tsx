...
export default function PrintBlankEnrolmentPage() {
  ...
  return <main className="db-public-page blank-enrolment-print"><section className="db-card blank-form-sheet">
    <header className="blank-form-header">{data.school?.logo_url ? <img src={data.school.logo_url} alt={`${data.school.school_name || "School"} logo`} /> : <div className="blank-logo-placeholder">School logo</div>}<div><h1>{data.school?.school_name || "School"}</h1><p>{configuration.form_title || "Enrolment Form"}</p>{data.school?.physical_address ? <p>{data.school.physical_address}</p> : null}{data.school?.contact_number || data.school?.email_address ? <p>{[data.school.contact_number, data.school.email_address].filter(Boolean).join(" · ")}</p> : null}</div><button className="db-button-primary no-print" type="button" onClick={() => window.print()}>Print form</button></header>
    ...
    <section><h2>Health and medical information</h2><div className="blank-form-grid"><Line label="Allergies" wide /><Line label="Medical conditions" wide /><Line label="Medical aid name" /><Line label="Membership number" /><Line label="Main member" /><Line label="Preferred doctor" /><Line label="Doctor contact number" /></div></section>
