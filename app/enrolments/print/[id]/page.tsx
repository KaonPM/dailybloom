...
export default function PrintBlankEnrolmentPage() {
  ...
  return <main className="db-public-page blank-enrolment-print"><section className="db-card blank-form-sheet">
    <header className="blank-form-header"><div aria-hidden="true" /><div className="blank-form-brand">{data.school?.logo_url ? <img src={data.school.logo_url} alt={`${data.school.school_name || "School"} logo`} /> : <div className="blank-logo-placeholder">School logo</div>}<h1>{data.school?.school_name || "School"}</h1><p>{configuration.form_title || "Enrolment Form"}</p><p><strong>Registration / EMIS / NPO number:</strong> {data.school?.registration_number || "Not provided by the school"}</p><p><strong>Registered address:</strong> {data.school?.physical_address || "Not provided by the school"}</p></div><div className="blank-form-contacts"><p><strong>Contact number:</strong> {data.school?.contact_number || "Not provided by the school"}</p><p><strong>Email:</strong> {data.school?.email_address || "Not provided by the school"}</p></div></header>
