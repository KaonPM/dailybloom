...
type PrintableEnrolment = {
  ...
  school?: { school_name?: string | null; logo_url?: string | null; contact_number?: string | null } | null;
...
export default function PrintBlankEnrolmentPage() {
  ...
  const documents = secondGuardianRequired && !data.documents.some((item) => item.title === "Second parent/guardian identification document") ? [...data.documents, { title: "Second parent/guardian identification document", instructions: "Attach a clear copy for the required second guardian.", is_required: true }] : data.documents;

  return <main className="db-public-page blank-enrolment-print"><section className="db-card blank-form-sheet">
    <header className="blank-form-header">{data.school?.logo_url ? <img src={data.school.logo_url} alt={`${data.school.school_name || "School"} logo`} /> : <div className="blank-logo-placeholder">School logo</div>}<div><h1>{data.school?.school_name || "School"}</h1><p>{configuration.form_title || "Enrolment Form"}</p>{data.school?.contact_number ? <p>Contact: {data.school.contact_number}</p> : null}</div><button className="db-button-primary no-print" type="button" onClick={() => window.print()}>Print form</button></header>
    ...
    <section><h2>Parent or guardian details</h2><div className="blank-form-grid"><Line label="Full name" /><Line label="Relationship to learner" /><Line label="Contact mobile number" /><Line label="Contact number during the day" /><Line label="Parent Portal mobile number" /><Line label="Email address" /><Line label="Employer or business" /><Line label="Occupation" /><Line label="Work contact number" /><Line label="Home address" wide /></div></section>
    {configuration.second_guardian_mode !== "hidden" ? <section><h2>Second parent or guardian{secondGuardianRequired ? " (required)" : " (optional)"}</h2><div className="blank-form-grid"><Line label="Full name" /><Line label="Relationship to learner" /><Line label="Mobile number" /><Line label="Contact number during the day" /><Line label="Employer or business" /><Line label="Occupation" /></div></section> : null}
    ...
    <section><h2>Health information</h2><Line label="Allergies, medical aid or medical notes" wide /></section>
    ...
    <section><h2>Required documents</h2><div className="blank-check-list">{documents.map((item) => <p key={item.title}>☐ <strong>{item.title}{item.is_required ? " *" : ""}</strong>{item.instructions ? ` — ${item.instructions}` : ""}</p>)}</div></section>
