...
export default function PlatformTrustSecurityPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const role = String(profile.platform_role || profile.role || "").toLowerCase();
      const canView = role === "master" ||
        (role === "master_admin" && profile.permissions?.includes(PERMISSIONS.PLATFORM_REPORTS_VIEW));
  ...
  return <TrustSecurityCentre scope="platform" />;
}
