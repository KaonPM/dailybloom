          ...
          <section className="db-card" style={{ padding: 24 }}>
                    ...
                    <tr key={record.id} style={{ borderTop: "1px solid var(--db-border, #eadfd8)" }}>
                      ...
                      <td style={{ padding: 12 }}>
                        {record.status === "submitted" ? (
                          ...
                          record.next_classroom_id ? <span>Classroom planned</span> : rolloverAvailable ? <Link href={`/awaiting-classroom-allocation${schoolQuery}`}>Awaiting classroom allocation</Link> : <span>Available for allocation from 1 January {campaign?.school_year}</span>
            ...
            {selectedForReview ? (
              <section className="db-soft-card" style={{ padding: 18, marginTop: 20 }} aria-label="Review re-enrolment">
                ...
                {selectedForReview.status === "submitted" ? <><label style={{ display: "block", marginTop: 16 }}><span className="db-label">If returning the form, explain what the parent needs to correct</span><textarea className="db-input" value={declineReason} onChange={(event) => setDeclineReason(event.target.value)} rows={3} placeholder="For example: Please upload the updated immunisation card before re-enrolment can be approved." /></label><div className="db-page-actions" style={{ marginTop: 16 }}><button className="db-button-primary" type="button" disabled={reviewing} onClick={() => void reviewReenrolment("approve_reenrolment")}>{reviewing ? "Saving…" : "Approve Re-enrolment"}</button><button className="db-button-secondary" type="button" disabled={reviewing || !declineReason.trim()} onClick={() => void reviewReenrolment("decline_reenrolment")}>Return to Parent for Updates</button></div></> : <div className="db-success-banner" style={{ marginTop: 16 }}>This approved submission is locked as a historical snapshot. Edit the live learner profile separately if a later correction is required.</div>}
              </section>
            ) : null}
          </section>
        </>
