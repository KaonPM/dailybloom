        ...
        {(["0_2", "2_6"] as RequirementTemplateKey[]).map((templateKey) => {
          ...
          return <details key={templateKey} className="db-soft-card" style={{ padding: 12 }}>
            ...
            <div style={{ display: "grid", gap: 10, marginTop: 12 }}>
              ...
              {(["stationery", "hygiene"] as const).map((category) => { const categoryItems = templateItems.filter((item) => item.category === category); return <section key={category} style={{ display: "grid", gap: 8 }}><h4 style={{ margin: "6px 0 0", textTransform: "capitalize" }}>{category} ({categoryItems.length})</h4>{categoryItems.length ? categoryItems.map((item) => <div className="db-soft-card" key={item.id} style={{ padding: 10, display: "flex", justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}><span><strong>{item.item_name}</strong><br /><small className="db-helper">{item.quantity ? `Quantity: ${item.quantity}` : "No quantity"} {item.is_required ? "· Required" : ""} {!item.is_active ? "· Inactive" : ""}</small></span><span style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><button className="db-collapse-action" type="button" onClick={() => void editRequirementTemplate(item)}>Edit</button>{item.is_active ? <button className="db-collapse-action" type="button" onClick={() => void archiveRequirementTemplate(item.id)}>Deactivate</button> : <button className="db-collapse-action" type="button" onClick={() => void saveRequirementTemplate({ ...item, is_active: true })}>Activate</button>}<button className="db-collapse-action" type="button" onClick={() => void deleteSetupItem("requirement", item.id)}>Delete</button></span></div>) : <p className="db-helper" style={{ margin: 0 }}>No {category} items in this template yet.</p>}</section>; })}
