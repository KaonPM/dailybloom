-- Record an explicit outcome for every active learner when an activity is
-- completed. Learners omitted from the support and strength selections meet
-- the standard for that learning opportunity.
...
begin
  ...
  select
    p_school_id, v_plan.classroom_id, learner.id, p_plan_id,
    v_plan.developmental_area, v_plan.theme, v_plan.activity_date,
    v_plan.activity_name, 'meeting_expectations', null, v_actor_id
  from public.learners learner
  where learner.school_id = p_school_id
    and learner.classroom_id = v_plan.classroom_id
    and coalesce(learner.is_deleted, false) = false
    and not exists (
      select 1
      from jsonb_to_recordset(coalesce(p_support_rows, '[]'::jsonb)) as support(learner_id uuid)
      where support.learner_id = learner.id
    )
    and not exists (
      select 1
      from jsonb_to_recordset(coalesce(p_strength_rows, '[]'::jsonb)) as strength(learner_id uuid)
      where strength.learner_id = learner.id
    );
